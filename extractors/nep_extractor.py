...
class NEPExtractor(BaseExtractor):
    ...
    def extract_student_info(self, text):
        ...
        for line in lines:
            ...
            # Extract Name
            name_match = re.search(r"Student'?s?\s*Name\s*:\s*([A-Za-z\s]+)", line_clean, re.IGNORECASE)
            if not name_match:
                name_match = re.search(r"Student\s*Name\s*:\s*([A-Za-z\s]+)", line_clean, re.IGNORECASE)
            if name_match:
                student_info['name'] = name_match.group(1).strip()
            
            # Extract Mother's Name
            mother_match = re.search(r"Mother'?s?\s*Name\s*:\s*([A-Za-z\s]+)", line_clean, re.IGNORECASE)
            if mother_match:
                student_info['mother_name'] = mother_match.group(1).strip()
            
            # Extract Registration No
            reg_match = re.search(r"(?:Registration|Regd\.?)\s*No\.?\s*:\s*(\d+)", line_clean, re.IGNORECASE)
            if reg_match:
                student_info['registration_no'] = reg_match.group(1).strip()
            
            # Extract Program
            prog_match = re.search(r"Programme?\s*:\s*([A-Za-z\s]+)", line_clean, re.IGNORECASE)
            if prog_match:
                student_info['program'] = prog_match.group(1).strip()
            
            # Extract Department
            dept_match = re.search(r"Department\s*:\s*([A-Za-z\s]+)", line_clean, re.IGNORECASE)
            if dept_match:
                student_info['department'] = dept_match.group(1).strip()
            
            # Extract Semester
            sem_match = re.search(r"Semester\s*:\s*([IVXivx\d]+)", line_clean, re.IGNORECASE)
            if sem_match:
                student_info['semester'] = sem_match.group(1).strip()
            
            # Extract Examination
            exam_match = re.search(r"Examination\s*:\s*(.+)", line_clean, re.IGNORECASE)
            if exam_match:
                student_info['examination'] = exam_match.group(1).strip()
